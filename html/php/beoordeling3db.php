<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $schimmelachtigSmaak == '' || $wortGeur == '' || $wortSmaak == '' || $zwaveligGeur == '' || $zwaveligSmaak == '')
        {
            ...
            $query = "INSERT INTO geurafwijkingen(groeneAppel, azijnzuur, olie, branderig, chloor, dms, fenolen, muf, 
            nootachtig, lichtsmaak, metaalachtig, oplosmiddel, schimmelachtig, harsachtig, zwavelig, andere, waardeAndere) 
            VALUES 
            ('$acetaldehydeGeur', '$azijnzuurGeur', '$boterGeur', '$brandigGeur', '$chloorGeur', '$dmsGeur', '$fenolenGeur', '$geoxideerdGeur',
            '$grasachtigGeur', '$lichtsmaakGeur', '$metaalachtigGeur', '$oplosmiddelGeur', '$schimmelachtigGeur', '$wortGeur', '$zwaveligGeur', '$afwijkingenAndere', '$afwijkingenAndereGeur')";
            ...
            $afwijkingId = $data['id'];
            $query = "INSERT INTO geur(punten, neutraalId, afwijkingenId) VALUES ('$puntenGeur', '$geurId', '$afwijkingId')";
            mysql_query($query) or die ('Error updating' . mysql_error());
            ...
            $query = "INSERT INTO smaakafwijkingen(groeneAppel, azijnzuur, olie, branderig, chloor, dms, fenolen, muf, 
            nootachtig, lichtsmaak, metaalachtig, oplosmiddel, schimmelachtig, harsachtig, zwavelig, andere, waardeAndere) 
            VALUES 
            ('$acetaldehydeSmaak', '$azijnzuurSmaak', '$boterSmaak', '$brandigSmaak', '$chloorSmaak', '$dmsSmaak', '$fenolenSmaak', '$geoxideerdSmaak',
            '$grasachtigSmaak', '$lichtsmaakSmaak', '$metaalachtigSmaak', '$oplosmiddelSmaak', '$schimmelachtigSmaak', '$wortSmaak', '$zwaveligSmaak', '$afwijkingenAndere', '$afwijkingenAndereSmaak')";
            mysql_query($query) or die ('Error updating' . mysql_error());
            ...
            $data = mysql_fetch_assoc($result);
            $afwijkingId = $data['id'];
            $query = "INSERT INTO smaak(punten, neutraalId, afwijkingenId) VALUES ('$puntenSmaak', '$smaakId', '$afwijkingId')";
            mysql_query($query) or die ('Error updating' . mysql_error());
            ...
            $query = "UPDATE beoordeling SET geurId = $geur, smaakId = $smaak WHERE voorproeverId = '$voorproever' AND bierId = '$bierId'";
            mysql_query($query) or die ('Error updating' . mysql_error());
